...
for ((i=1; i<=8; i++)); do
    ...
    docker network disconnect -f p3_red_web $container_name
done
...
for ((i=1; i<=8; i++)); do
    ...
    docker network disconnect -f p3_red_servicios $container_name
done
...
for ((i=1; i<=8; i++)); do
    ...
    docker network connect --ip $container_ip p3_red_web $container_name
done
...
for ((i=1; i<=8; i++)); do
    ...
    docker network connect --ip $container_ip p3_red_servicios $container_name
done
